...
namespace Hallmanac.CryptoHelpers
{
    ...
    public class PasswordHashingSvc : IPasswordHashingSvc
    {
        ...
        public FunqResult<byte[]> ComputePasswordAndSaltBytes(byte[] salt, string password, int iterationCount = MinIterationRange)
        {
            ...
            {
                return FunqFactory.Fail($"The password length was greater than the maximum allowed. Please make it less than {MaxPasswordSize}", (byte[])null);
            }

            if (convertedPassword.Length < MinPasswordSize)
            {
                return FunqFactory.Fail($"The password length was less than the minimum allowed. Please make it greater than {MinPasswordSize}", (byte[])null);
            }
